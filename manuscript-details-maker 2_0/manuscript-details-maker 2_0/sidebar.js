...
function buildSectionText(sectionId){
  ...
  if(sectionId==='submissionChecks'){
    ...
    if(transOpt || transTxt) lines.push('Transfer: ' + [transOpt, transTxt].filter(x=>x).join(' - '));
...
[
 ...
 //
    ...
    // Logic for final decision unchanged
  }
...
function copySection(sectionId, statusId){
  ...
  if(!text){ showStatus(statusId, 'Nothing to copy', 2000); return; }
...
function resetForm(){
  ...
  document.querySelectorAll('input,textarea,select').forEach(el=>{
    ...
    if(el.classList.contains('ndjField')) { el.value = masterName + ' ' + masterDate + ' ' + masterJournal; return; }
